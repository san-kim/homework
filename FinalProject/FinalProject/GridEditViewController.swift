...
class GridEditViewController: UIViewController,EngineDelegate
{
    ...
    @IBAction func displayPressed(sender: AnyObject)
    {
        if StandardEngine.sharedInstance.changed
        {
            ...
            let alert = UIAlertController(title: "Exit Without Saving", message: "Are you sure you want to leave without saving?", preferredStyle: UIAlertControllerStyle.Alert)
            //add cancel button alert
            alert.addAction(UIAlertAction(title: "Cancel", style: UIAlertActionStyle.Default, handler: nil))
            alert.addAction(UIAlertAction(title: "Leave", style: UIAlertActionStyle.Default, handler: {(action) -> Void in self.tabBarController?.selectedIndex = 1}))
